...
const calcRegion = (gpsPhotos) => {
  const points = gpsPhotos
    .map(p => p.markerGps ?? p.gps)
    .filter(Boolean);
...
export default function DiaryScreen() {
  ...
  const mapSignature = mapMarkers
    .map(item => `${item.id}-${item.markerGps.latitude.toFixed(5)}-${item.markerGps.longitude.toFixed(5)}`)
    .join('|');
  ...
  useEffect(() => {
    ...
    setSelectedEmotion(savedDiary.emotion ? EMOTIONS.indexOf(savedDiary.emotion) : null);
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
          ...
          {gpsPhotos.length > 0 && Platform.OS === 'web' && (
            <View style={styles.webMapFallback}>
              ...
              <Text style={styles.webMapFallbackTitle}>웹에서는 사진 위치 타임라인만 표시돼요.</Text>
...
const styles = StyleSheet.create({
  ...
  photo: {
    ...
    borderRadius: 16, backgroundColor: '#F1F1F1',
  },
  removeBtn: {
    position: 'absolute',
    top: 8,
  ...
  saveBtn: {
    ...
    backgroundColor: '#1D9E75', justifyContent: 'center', alignItems: 'center',
  },
